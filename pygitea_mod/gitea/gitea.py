...
class Gitea:
    ...
    def modify_team(
        ...
        result = self.requests_patch(
            Gitea.MODIFY_TEAM % teamID,
            data=modifications,
        )

        if "id" in result:
            self.logger.info("Successfully modified Team %s" % result["name"])
        else:
            self.logger.error("Team not modified... (gitea: %s)" % result["message"])
            self.logger.error(result["message"])
            raise Exception("Team not modified... (gitea: %s)" % result["message"])
        api_object = Team.parse_response(self, result)
        pass
        return api_object
